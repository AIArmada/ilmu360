...
| Institution and venue addresses | `Address` with `country_id` plus `admin_area_1_id` through `admin_area_4_id`, not fixed `state_id` / `district_id` / `subdistrict_id` columns. |
