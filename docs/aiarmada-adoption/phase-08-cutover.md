...
- Membership handling is now aligned with package-native behavior: invitation wrapper sits on the installed package model with hashed tokens. `MembershipClaim` uses `membership_applications` table (package-owned). Claim approve/reject/cancel actions use `ApplicationStatus` enum.
- Engagement cutover complete: registration, bookmark/save, and follow/unfollow all use package actions via `RegistrationService`, `EngagementManager`. Local `RegisterForEventAction`, `SaveEventAction`, `UnsaveEventAction`, and `HasFollowers` concern are deleted.
- Communications inbox cutover complete: `NotificationMessage` model deleted, `notifications` table dropped. Custom `InboxChannel` writes to `notification_inboxes`, Livewire/API reads from package `NotificationInbox` model. `auto_capture` enabled.
- Remaining work is no longer basic runtime compatibility. The next packet is structural: replace notification pipeline/engine, custom channels (FCM, WhatsApp), and digest scheduling, then widen package-first API/MCP/directory verification ahead of the final 8.H gate.
