...
ilmu360° exposes two web MCP servers plus two local testing handles that AI clients can connect to:
...
| Raw HTTP admin API | `docs/ilmu360_mobile_api_reference.md` and `/api/v1/admin` | HTTP admin contracts, schemas, and record payloads | MCP write-schema parity |
...
```shell
php artisan mcp:inspector ilmu360-admin-local
php artisan mcp:inspector ilmu360-member-local
```
...
The broader internal cross-surface parity docs (`ilmu360_api_mcp_filament_crud_comparison.*`) are intentionally not exposed through MCP.
...
- fetch the matching server guide directly when the question is clearly about ilmu360° MCP docs
...
- Ratio is fixed by target and enforced server-side:
  ...
  - `poster` = `3:4` (external/social flyer visual)
...
- Institution write schemas now expose additional field semantics for `names` (multi-name array), `address`, `contacts`, and `social_media`.
...
- On direct MCP institution writes, use the `names` array with items `{full_name, name_type (nickname|abbreviation|local|historical), language_code, is_primary}`. `names: []` clears all alternative names; omitted `names` preserves existing.
...
|---|---|---|
| `search` | Search the verified ilmu360° MCP documentation pages exposed by this server | MCP-only documentation discovery tool |
| `fetch` | Fetch one verified ilmu360° documentation page by id | MCP-only documentation fetch tool |
...
| `admin-upload-event-poster-image` | Upload and save a pre-generated 3:4 portrait marketing poster for one event. Use `admin-event-poster-image-prompt` to build the prompt and reference images first, then generate with ChatGPT, then call this tool with the image descriptor. | MCP-only event poster upload tool |
...
| Tool | Purpose |
|---|---|
| `search` | Search the verified ilmu360° MCP documentation pages exposed by this server |
| `fetch` | Fetch one verified ilmu360° documentation page by id |
...
| `member-upload-event-poster-image` | Upload and save a pre-generated 3:4 portrait marketing poster for one accessible event. Use `member-event-poster-image-prompt` to build the prompt and reference images first, then generate with ChatGPT, then call this tool with the image descriptor. |
...
| `member-upload-event-poster-image` | Upload a pre-generated image to the accessible event `poster` collection at `3:4` (use `member-event-poster-image-prompt` first) | `event_key`, `image`, `creative_direction?` |
...
- For tiny call examples, see `docs/ilmu360_mcp_tool_examples.md`.
