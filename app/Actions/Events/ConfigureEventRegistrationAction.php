...
final class ConfigureEventRegistrationAction
{
    use AsAction;
    ...
    public function handle(
        ...
        $normalizedTickets = $this->normalizeTickets($tickets, $pricingMode);
        $this->validateSeatingConfiguration($normalizedTickets, $seating);
        ...
        if (! $occurrence instanceof EventOccurrence) {
            throw new InvalidArgumentException('The event occurrence could not be configured.');
        }
        ...
        foreach ($normalizedTickets as $ticket) {
        ...
        if ($requiresSeating) {
            ...
            $this->validateSeatingCapacity($normalizedTickets, $seatMap);
            $this->attachGeneralAdmissionSections($ticketTypes, $normalizedTickets, $seatMap);
        }

        return $event->fresh(['primaryOccurrence', 'ticketTypes', 'accessPolicies']) ?? $event;
    }
    ...
    private function normalizeTickets(array $tickets, PricingMode $pricingMode): array
    {
        ...
        foreach (array_values($tickets) as $index => $ticket) {
            ...
            $seatingMode = SeatingMode::tryFrom((string) ($ticket['seating_mode'] ?? SeatingMode::None->value));
            ...
            $normalized[] = [
                ...
                'seating_mode' => $seatingMode instanceof SeatingMode
                    ? $seatingMode->value
    ...
    /**
     * @param  array<int, array{seating_mode: string}>  $tickets
     * @param  array<string, mixed>  $seating
     */
    private function validateSeatingConfiguration(array $tickets, array $seating): void
    {
        $seatingModes = array_values(array_filter(
            array_map(static fn (array $ticket): string => $ticket['seating_mode'], $tickets),
    ...
    /** @param  list<array{quota: ?int, seating_mode: string}>  $tickets */
