...
use App\Mcp\Prompts\MemberEventCoverImagePrompt;
use App\Mcp\Prompts\MemberEventPosterImagePrompt;
...
#[Name('ilmu360-member')]
#[Version('1.0.0')]
#[Instructions('Authenticated member MCP server for Ahli-scoped institutions, persons, references, and related events. Supports scoped resource discovery, record reads, record-specific next-step MCP actions, schema-guided updates for writable Ahli records when the authenticated member has the matching scoped permissions, plus member-side workflow tools for contribution-request queues and membership applications that mirror the Ahli dashboard. Event cover images are fixed at 16:9 and event poster images are fixed at 3:4. Media fields use JSON base64 file descriptors when advertised by the schema. To generate event images: use the `member-event-cover-image-prompt` or `member-event-poster-image-prompt` prompts to get the engineered prompt text and brand reference images, generate the image using ChatGPT native image generation, then upload the result with `member-upload-event-cover-image` or `member-upload-event-poster-image`. Entity-selection hint for record search: institution-type nouns (`masjid`, `surau`, `madrasah`, `maahad`, `pondok`, `sekolah`, `kolej`, `universiti`) should be searched as `institutions` first; venue-type nouns (`dewan`, `auditorium`, `stadium`, `perpustakaan`, `padang`, `hotel`) should be searched as `venues` first; `spaces` are finer-grained sublocations inside institutions and should not be the default first lookup for named mosques or surau. The server exposes the primary member MCP guide as a raw markdown resource, read-only `search` / `fetch` documentation tools that expose the broader verified member docs catalog for tool-centric clients such as ChatGPT and the OpenAI Responses MCP integration, plus a `documentation-tool-routing` prompt that explains when to use the guide and the documentation tools. Operational MCP tool calls are rejected until `docs-member-mcp-guide` has been fetched or the guide resource has been read in the current initialized MCP session. Designed for bearer-token clients such as VS Code, ChatGPT, Gemini, Grok, Claude, and Opencode.')]
class MemberServer extends Ilmu360Server
{
    ...
    protected array $prompts = [
        ...
        MemberEventCoverImagePrompt::class,
        MemberEventPosterImagePrompt::class,
    ];
    ...
    protected array $tools = [
        ...
        MemberListMembershipApplicationsTool::class,
        MemberSubmitMembershipApplicationTool::class,
        MemberCancelMembershipApplicationTool::class,
