...
- If it is not already in context, call `fetch` with `docs-member-mcp-guide` directly when the task is clearly about ilmu360 member MCP behavior, or call `search` first when the topic is still fuzzy.
- Use `fetch` first when the question is clearly about ilmu360 member MCP behavior, because `docs-member-mcp-guide` is the primary starting guide in the broader verified member MCP docs catalog.
...
TEXT,
            $this->availableDocumentationPages(),
...
- If the question is about the exposed ilmu360 member MCP docs, go straight to `docs-member-mcp-guide`.
...
- Prefer `docs-member-mcp-guide` first, because it is the canonical starting guide and includes the capability matrix and writable resource summaries.
TEXT,
        };
    }

    private function availableDocumentationPages(): string
    {
        $pages = array_map(
            static fn (array $document): string => sprintf('- `%s` — %s', $document['id'], $document['title']),
            $this->documentationCatalog->all(),
        );
        ...
        return implode("\n", [
            'Additional verified page ids exposed through `search` and `fetch` include:',
            ...$pages,
        ]);
    }
