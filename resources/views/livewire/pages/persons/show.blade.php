...
@section('meta_robots', (in_array((string) $person->status, ['verified', 'pending'], true) && $person->speaker_status === \App\Enums\SpeakerStatus::Active) ? 'index, follow' : 'noindex, nofollow')
                ...
                <section class="scroll-reveal reveal-up revealed">
                    ...
                    >
                        @foreach($upcomingEvents as $event)
                            @include('livewire.pages.persons.partials.event-card', [
                                'event' => $event,
                                'roleLabel' => null,
                                'wireKey' => 'upcoming-'.$event->id,
                            ])
                        @endforeach
                    ...
                    <section class="scroll-reveal reveal-up revealed">
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                            <div>
                                ...
                                <h2 class="mt-1 font-heading text-3xl font-bold text-emerald-950">{{ __('Peranan Lain Dalam Majlis') }}</h2>
                            </div>

                            <span class="inline-flex w-fit shrink-0 items-center gap-2 rounded-full border border-violet-200 bg-violet-50 px-3 py-1.5 text-xs font-bold text-violet-800">
                                <span class="relative flex h-2 w-2">
                                ...
                                {{ number_format($otherRoleParticipations->count()) }}
                            </span>
                        </div>
                            ...
                            @foreach($otherRoleParticipations as $keyPerson)
                                ...
                                @if($event)
                                    @include('livewire.pages.persons.partials.event-card', [
                                        'event' => $event,
                                        'roleLabel' => $roleLabel,
                                        'wireKey' => 'role-participation-'.$keyPerson->id,
                                    ])
                                @endif
                            @endforeach
                        </div>
