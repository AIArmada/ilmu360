                ...
                <div class="space-y-5">
                    ...
                    <div class="max-w-3xl space-y-3">
                        <h1 class="font-heading text-balance text-4xl font-semibold tracking-[-0.04em] text-white sm:text-5xl lg:text-6xl">
                            {{ __('Create the event, then add sessions to its occurrence.') }}
                        </h1>
                        <p class="max-w-2xl text-sm leading-7 text-white/72 sm:text-base">
                            {{ __('This builder creates the event and its first occurrence. Additional sessions use the standard Hantar Majlis workflow and belong to that occurrence.') }}
                        </p>
                    </div>
                    ...
                    <div class="grid gap-3 sm:grid-cols-3">
                        <div class="rounded-2xl border border-white/10 bg-white/6 px-4 py-4 backdrop-blur-sm">
                            ...
                            <p class="mt-2 text-sm font-medium text-white">{{ __('Create one event draft') }}</p>
                        </div>
                        <div class="rounded-2xl border border-white/10 bg-white/6 px-4 py-4 backdrop-blur-sm">
                        ...
                        <div class="rounded-2xl border border-white/10 bg-white/6 px-4 py-4 backdrop-blur-sm">
                            ...
                            <p class="mt-2 text-sm font-medium text-white">{{ __('Each submission becomes an occurrence session') }}</p>
                        </div>
                    </div>
                </div>
                ...
                @if($activeStep === 1)
                    <section class="overflow-hidden rounded-[2rem] border border-white/80 bg-white/90 shadow-[0_24px_60px_-35px_rgba(15,23,42,0.35)] backdrop-blur">
                        <div class="border-b border-slate-200/80 bg-[linear-gradient(135deg,rgba(15,23,42,0.98),rgba(15,118,110,0.96),rgba(217,166,76,0.25))] px-6 py-6 text-white sm:px-8">
                            ...
                            <p class="mt-2 max-w-2xl text-sm leading-7 text-white/75">{{ __('This step defines the event and its first occurrence. Sessions are submitted one by one afterwards through the standard event form.') }}</p>
                        </div>
                            ...
                            <div class="rounded-3xl border border-dashed border-emerald-300 bg-emerald-50/70 p-5">
                                <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                    <div class="max-w-2xl">
                                        ...
                                        <h3 class="mt-2 text-xl font-semibold text-slate-900">{{ __('Use a template, then submit sessions individually.') }}</h3>
                                        <p class="mt-2 text-sm leading-6 text-slate-600">{{ __('Templates scaffold the event title, description, and occurrence timeframe. Detailed sessions come later through the normal submission UI.') }}</p>
                                    </div>
                                </div>
                ...
                @if($activeStep === 2)
                    <section class="overflow-hidden rounded-[2rem] border border-white/80 bg-white/90 shadow-[0_24px_60px_-35px_rgba(15,23,42,0.35)] backdrop-blur">
                        <div class="border-b border-slate-200/80 bg-slate-950 px-6 py-6 text-white sm:px-8">
                            ...
                            <h2 class="mt-3 font-heading text-3xl font-semibold tracking-[-0.03em]">{{ __('Set the occurrence window and defaults.') }}</h2>
                            <p class="mt-2 max-w-2xl text-sm leading-7 text-white/72">{{ __('These defaults establish the event and its first occurrence. Sessions carry their own titles, schedules, and content.') }}</p>
                        </div>
                        ...
                        <div class="grid gap-6 px-6 py-6 sm:px-8 sm:py-8 xl:grid-cols-[1.2fr_0.8fr]">
                            ...
                            <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-500">{{ __('Normalized Session Workflow') }}</p>
                                <div class="mt-4 space-y-3">
                                    <div class="rounded-2xl bg-white px-4 py-3 shadow-sm">
                                        <p class="text-xs uppercase tracking-[0.22em] text-slate-400">{{ __('After event creation') }}</p>
                                        <p class="mt-2 text-sm font-semibold text-slate-900">{{ __('You will be redirected to Hantar Majlis to create the first session.') }}</p>
                                    </div>
                                    <div class="rounded-2xl bg-white px-4 py-3 shadow-sm">
                                        <p class="text-xs uppercase tracking-[0.22em] text-slate-400">{{ __('Each session') }}</p>
                                        <p class="mt-2 text-sm font-semibold text-slate-900">{{ __('Uses the normal event submission UI and belongs to the selected occurrence.') }}</p>
                                    </div>
                                    <div class="rounded-2xl bg-slate-950 px-4 py-3 text-white shadow-sm">
                                        ...
                                        <p class="mt-2 text-sm font-medium text-white/90">{{ __('No bulk session creation here. The package occurrence/session model is the single scheduling path.') }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                @if($activeStep === 3)
                    <section class="overflow-hidden rounded-[2rem] border border-white/80 bg-white/90 shadow-[0_24px_60px_-35px_rgba(15,23,42,0.35)] backdrop-blur">
                        <div class="border-b border-slate-200/80 bg-white px-6 py-6 sm:px-8">
                            ...
                            <h2 class="mt-3 font-heading text-3xl font-semibold tracking-[-0.03em] text-slate-950">{{ __('Create the event, then add occurrence sessions.') }}</h2>
                            <p class="mt-2 max-w-2xl text-sm leading-7 text-slate-500">{{ __('This page creates the event and first occurrence. Session work begins on the standard submit-event page immediately after.') }}</p>
                        </div>

                        <div class="space-y-6 px-6 py-6 sm:px-8 sm:py-8">
                            <div class="grid gap-4 xl:grid-cols-3">
                                <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                                    <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-500">{{ __('Event') }}</p>
                                    <h3 class="mt-3 text-xl font-semibold text-slate-900">{{ trim((string) ($form['title'] ?? '')) ?: __('Untitled Event') }}</h3>
                                    <p class="mt-2 text-sm leading-6 text-slate-500">{{ trim((string) ($form['description'] ?? '')) ?: __('No event description yet.') }}</p>
                                </div>
                                <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                                ...
                                <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                                    ...
                                    <p class="mt-3 text-sm font-semibold text-slate-900">{{ __('1 event with 1 occurrence') }}</p>
                                    <p class="mt-1 text-sm text-slate-600">{{ __('Then redirect to Hantar Majlis for the first session.') }}</p>
                                    <p class="mt-4 text-xs uppercase tracking-[0.18em] text-slate-400">{{ __('Attachment model') }}</p>
                                    <p class="mt-1 text-sm text-slate-600">{{ __('Every session belongs to this event occurrence.') }}</p>
                                </div>
                            </div>
                            ...
                            <div class="rounded-[1.75rem] border border-slate-200 bg-slate-50 p-5">
                                <div class="flex items-center justify-between gap-4">
                                    <div>
                                        <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-500">{{ __('Occurrence Window') }}</p>
                ...
                <section class="rounded-[2rem] border border-white/80 bg-white/90 p-4 shadow-[0_24px_60px_-35px_rgba(15,23,42,0.35)] backdrop-blur sm:p-5">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex gap-3">
                            ...
                            @else
                                <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-emerald-600/20 transition hover:bg-emerald-700">{{ __('Create Event and Continue') }}</button>
                            @endif
                        </div>
                    </div>
                </section>
                ...
                <div class="space-y-6">
                    <section class="overflow-hidden rounded-[2rem] border border-slate-200 bg-slate-950 text-white shadow-[0_24px_60px_-35px_rgba(15,23,42,0.55)]">
                        <div class="border-b border-white/10 px-5 py-5">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-gold-300">{{ __('Stable Session Workflow') }}</p>
                            <h2 class="mt-2 text-2xl font-semibold tracking-[-0.03em]">{{ __('What this page does now') }}</h2>
                            <p class="mt-2 text-sm leading-6 text-white/65">{{ __('It creates the event and its first occurrence. Additional sessions come through the normal event submission form.') }}</p>
                        </div>

                        <div class="grid gap-3 px-5 py-5 sm:grid-cols-2 lg:grid-cols-1">
                            <div class="rounded-2xl border border-white/10 bg-white/5 px-4 py-4">
                                <p class="text-[11px] uppercase tracking-[0.22em] text-white/45">{{ __('Event Output') }}</p>
                                <p class="mt-2 text-sm font-semibold text-white">{{ __('One draft event and occurrence') }}</p>
                            </div>
                            <div class="rounded-2xl border border-white/10 bg-white/5 px-4 py-4">
                                <p class="text-[11px] uppercase tracking-[0.22em] text-white/45">{{ __('Session Flow') }}</p>
                            ...
                            <div class="rounded-2xl border border-white/10 bg-white/5 px-4 py-4">
                                ...
                                <p class="mt-2 text-sm font-semibold text-white">{{ __('Attached through the event occurrence') }}</p>
                            </div>
                        </div>
                    </section>
                    ...
                    <section class="rounded-[2rem] border border-white/80 bg-white/90 p-5 shadow-[0_24px_60px_-35px_rgba(15,23,42,0.35)] backdrop-blur">
                        ...
                        <div class="mt-4 space-y-3 text-sm leading-6 text-slate-600">
                            <p>{{ __('The session form already handles real event details and mature validation. Reusing it reduces hidden edge cases.') }}</p>
                            <p>{{ __('The advanced builder focuses on event identity and its first occurrence instead of duplicating session editing.') }}</p>
                        </div>
                    </section>
                </div>
            </aside>
