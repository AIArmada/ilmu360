...
- Treat CHIP event names and resource statuses as separate fields: the official `payout.failed` event carries payout status `error`; verify both against the provider schema before changing status predicates.
- When a package still persists and tests a configuration shape such as `upline_levels[].share`, it is an active contract; do not silently replace it with zero while removing unrelated legacy shapes.
- A limit sentinel must be consistent at both advisory helpers and the mutation policy: zero disables and negative means unlimited, with tests through both the helper and the actual policy.
...
- When running Pest with Paratest, pass one test path per invocation; multiple paths can be interpreted as excess CLI arguments, and concurrent invocations can produce misleading temporary-result warnings.
- State-machine transitions expose status objects as concrete state instances; use `instanceof` against the target state rather than assuming a generic `status->is()` helper exists.
- Package-level payment/refund fixtures must include the same generic inventory and pricing attributes used by production ticket types; otherwise a green-looking unit path can miss the checkout integration contract.
- Provider webhooks should emit a provider-neutral domain event only after the provider-specific payload is verified and normalized; application refund completion can then remain independent of CHIP while preserving the current owner context.
- A replaceable payment provider needs a package-owned registration seam, not only a configurable default string: discover tagged `PaymentProcessorInterface` implementations before resolving the checkout default, and keep the processor identifier separate from the concrete provider used for currency-aware follow-up operations.
- When a checkout supports future currencies, propagate the request currency through every billable adapter path; a model-level `preferredCurrency()` default must be an explicit fallback, not an accidental override of the checkout’s currency snapshot.
- When a user corrects an external project path, verify the corrected workspace before reading credentials or reporting a sandbox blocker; the local sandbox is `/Users/Saiffil/Herd/unfair`.
- Public checkout result pages must enter explicit global Commerce owner context while loading package-owned contacts, orders, registrations, or passes; an authenticated user does not imply an owner context.
- When an unreleased contract is intentionally hard-cut, do not add an optional capability or compatibility shim to accommodate stale implementations; update every in-repository implementation and test double, then run the affected suite so hidden implementers are found.
- When auditing a long-running feature implementation, scan the complete change set for explicit legacy customer paths, old payload-shape fallbacks, and unscoped historical metadata branches; remove them instead of only fixing the latest compiler error.
- When a new integration has a canonical metadata contract, accept that contract at the application boundary; do not silently search provider payloads or older order shapes for alternate locations.
- When removing compatibility, distinguish intentional public/API compatibility from legacy data, configuration, and payload behavior: preserve stable provider-neutral/API surfaces, but delete old storage keys, old payload shapes, deprecated facades, and migration-era fallbacks rather than retaining them under a broad compatibility label.
- When integrating CHIP Collect, do not infer payload or lifecycle shapes from generic payment conventions or existing adapters. Verify each request, response, webhook, signature, refund, and checkout field against the official CHIP API documentation before changing Commerce.
- For CHIP Collect, registered webhook deliveries and Purchase success callbacks are different channels: use `Webhook.public_key` for the former and the company `GET /public_key/` for the latter; never use a cross-channel key fallback.
- CHIP’s `overdue` status still allows payment, while only `hold` is capturable or releasable; preserve provider-native status vocabulary and map it at the universal payment boundary.
- Preserve CHIP’s canonical nested `purchase.total`, top-level `recurring_token`, and `event_type` payload shape; do not invent idempotency headers for mutation endpoints unless the provider documents them.
- When a package owns its webhook or environment configuration, remove reads of unrelated legacy config keys and old environment-name aliases; update callers and tests to the canonical package-owned settings.
- A hard-cut workflow must revalidate its current policy at the mutation boundary; a page-load check alone can be bypassed by changing the event configuration while the form is open.
- When auditing provider/package regressions under an explicit no-legacy requirement, verify the current canonical contract first and fix only genuine defects; never reintroduce removed payload aliases, status names, or compatibility fallbacks to satisfy a stale review.
- When an audit names schema columns in a shared package, verify the package's current core models, migrations, and data contracts before changing the application or adding migrations. A stale snapshot of an earlier addressing design can make an apparently canonical `admin_area_*` fix actively wrong.
- When reviewing shared Commerce changes, treat the package contract as authoritative: compare the complete diff with package callers and the application integration, keep only generic bug fixes/enhancements, and fix application-specific compatibility at the application boundary rather than reshaping the package.
- When a package exposes a configured model resolver, every adapter must use it consistently for its declared model, new records, and validation probes; one hardcoded base-model construction can bypass the host application’s model behavior.
