...
it('documents the public reference directory in the mobile api reference', function () {
    ...
    expect($markdown)
        ...
        ->toContain('`/speakers`, `/institutions`, and `/references` return **only** records where `status = \'verified\'`')
        ->toContain('Public reference directory list items expose `display_title`, `parent_reference_id`, `part_type`, `part_number`, `part_label`, `is_part`, `author`, `type`, `publisher`, `publication_year`, `events_count`, `front_cover_url`, and `is_following` by default.')
